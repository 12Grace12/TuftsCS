...
bool Maze::solve_maze_recursive(int row, int col) {
	...
	cout << char(maze[row][col]) << " @ " << row << ", " << col << "---" << maze_size << endl;

	if (row < 0 || col < 0 || row > maze_array_size || col > maze_array_size) {
	    return false;
	} else if (maze[row][col] == 'X' || maze[row][col] == '.' || maze[row][col] == 'b') {
	    return false;
	}
	...
	// north
	if (solve_maze_recursive(row-1,col)) {
		//cout << "north true @ " << row << col << endl;
		return true;
	}
	
	// east
	if (solve_maze_recursive(row,col+1)) {
		//cout << "east true @ " << row << col << endl;
		return true;
	}
	
	// south
	if (solve_maze_recursive(row+1,col)) {
		// cout << "south true @ " << row << col << endl;
		return true;
	}
	
	// west
	if (solve_maze_recursive(row,col-1)) {
		// cout << "west true @ " << row << col << endl;
		return true;
	}
