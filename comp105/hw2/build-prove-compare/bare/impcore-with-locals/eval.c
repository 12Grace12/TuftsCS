...
/* make [[f]] the function denoted by [[e->u.apply.name]], or call [[error]] 43c */
            ...
            case USERDEF:
                /* apply [[f.u.userdef]] and return the result 44b */
                {
                    ...
                    /* make new name and value lists for local and formals
                     * and pointers to traverse lists */  
                    Namelist  new_nl = mkNL(NULL, NULL);
                    Valuelist new_vl = mkVL(0, NULL);
                    Namelist  temp_nl = new_nl;
                    Valuelist temp_vl = new_vl;

                    /* copy names and values from locals to new lists */
                    for (n = 0; n < lengthNL(f.u.userdef.locals); n++) {
                        temp_nl->tl = mkNL(nthNL(f.u.userdef.locals, n), NULL);
                        temp_vl->tl = mkVL(0, NULL);
                        temp_nl = temp_nl->tl;
                        temp_vl = temp_vl->tl;
                    }

                    /* copy names and values from formals to new lists */
                    for (n = 0; n < lengthNL(f.u.userdef.formals); n++) {
                        temp_nl->tl = mkNL(nthNL(nl, n), NULL);
                        temp_vl->tl = mkVL(nthVL(vl, n), NULL);
                        temp_nl = temp_nl->tl;
                        temp_vl = temp_vl->tl;
                    }
