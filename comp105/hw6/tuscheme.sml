...
fun typeof (e, tyex_env, kind_env) = 
    let
        fun ty (LITERAL (NUM (n)))  = inttype 
          | ty (LITERAL (BOOL (n))) = booltype
          ...
          | ty (LITERAL (PAIR (v1, v2))) =
                let
                    val tau_1 = ty (LITERAL (v1))
                    val tau_2 = ty (LITERAL (v2))
                in if eqType(listtype tau_1, tau_2) 
                    then tau_2
                    else raise TypeError ("mixed types in list literal")
                end
          | ty (LITERAL (_)) = raise TypeError "invalid type use of closures or primitives" 
          | ty (VAR (x))            = find (x,tyex_env)
          | ty (IFX (e1, e2, e3))   =
                let
                    val tau_1 = ty e1
                    val tau_2 = ty e2
                    val tau_3 = ty e3
                in if eqType(tau_1, booltype) andalso eqType(tau_2, tau_3)
                   then tau_2
                   else raise TypeError ("condition in if has type " ^ typeString tau_1)
                end
          | ty (APPLY(e, es)) = 
                let
                    val formals = case (ty e) of
                                  (CONAPP (TYCON "function", [CONAPP (TYCON "argtuple", args), _])) => args 
                                | _ => raise TypeError "invalid formal types found in function apply" 
                    val result = case (ty e) of
                                  (CONAPP (TYCON "function", [CONAPP (TYCON "argtuple", _), result])) => result
                                | _ => raise TypeError "invalid result type found in function apply" 
                    val tau_list = map ty es
                in if formals = tau_list
                    then result
                    else raise TypeError "type mismatch between formals and actuals" 
                end
          | ty (LETX (LET, es, e)) =
                let
                    fun bind_name_exp ((name, exp), env) = bind (name, typeof (exp, env, kind_env), env)
                    val new_tyex_env = foldl bind_name_exp tyex_env es  
                in typeof (e, new_tyex_env, kind_env)
                end
          | ty (LETX (LETSTAR, [], e)) = ty e
          | ty (LETX (LETSTAR, (x::xs), e)) = ty (LETX (LET, [x], (LETX (LETSTAR, xs, e))))
          | ty (LAMBDA ((xs,e))) = 
                let
                    (* check that each of the types provided have valid kind of * (TYPE) *)
                    val valid_kind = foldl (fn ((n,t),b) => kindof (t, kind_env) = TYPE andalso b) true xs

                    (* bind provided names and types to create new type env *)
                    fun bind_name_type ((n,t), env) = bind (n, t, env)
                    val new_tyex_env = foldl bind_name_type tyex_env xs 

                    val result_tau    = typeof (e, new_tyex_env, kind_env)
                in if valid_kind
                    then funtype(map (fn ((n,t)) => t) xs, result_tau)
                    else raise TypeError "invalid kind(s) for type(s) used in lambda evaluation"
                end
          | ty (WHILEX (e1,e2)) = 
                let
                    val tau_1 = ty e1
                    val tau_2 = ty e2
                in if eqType(tau_1, booltype)
                    then unittype
                    else raise TypeError ("condition in while expression has type "
                                          ^ typeString tau_1 ^ ", which should be bool")
                end
          | ty (SET (x,e)) = 
                let
                    val tau_1 = ty (VAR(x))
                    val tau_2 = ty e
                in if eqType(tau_1, tau_2)
                    then tau_1
                    else raise TypeError ("set variable " ^ x ^ " of type " ^ typeString tau_1
                                          ^ " to value of type " ^ typeString tau_2)
                end 
          | ty (BEGIN ([])) = unittype 
          | ty (BEGIN (es)) = foldl (fn (e,t) => ty e) unittype es
          | ty (TYLAMBDA (ns,e)) =
                let
                    val new_kind_env = foldl (fn (n,env) => bind(n,TYPE,env)) kind_env ns
                    val tau = typeof (e, tyex_env, new_kind_env)
                in FORALL(ns, tau)
                end
          | ty (TYAPPLY  (e,ts)) = instantiate (ty e, ts, kind_env)   
    in ty e
    end

fun elabdef (d, tyex_env, kind_env) =
    case d of
        VAL (x, e) => (bind (x, typeof (e, tyex_env, kind_env), tyex_env), 
                       typeString (typeof (e, tyex_env, kind_env)))
      | EXP e      => elabdef (VAL ("it", e), tyex_env, kind_env)
      | VALREC (name, tau, e) =>
            let
                val new_tyex_env = bind(name, tau, tyex_env)
                val tau_e        = typeof(e, new_tyex_env, kind_env)
            in if eqType(tau_e, tau) then (new_tyex_env, typeString tau)
               else raise TypeError ("in valrec: expecting type " ^ typeString tau
                                     ^ "but got type " ^ typeString tau_e)
            end
      | DEFINE (name, tau, (xs,e)) =>
            let
                val (ns, ts) = ListPair.unzip(xs)
                val func_tau = funtype(ts, tau)
            in elabdef (VALREC(name,func_tau,(LAMBDA((xs,e)))), tyex_env, kind_env)
            end
