...
(* given a constraint, solve for a subst that would solve the constrait *)
(* first two patterns deal with TRIVIAL and conjunctions (c1 /\ c2) *)
fun solve (TRIVIAL)  = idsubst 
  ...
  (* rest of patterns match to different types of simple equailities (t1 ~ t2) *)
  ...
  | solve (CONAPP (n1,ts1) ~ CONAPP (n2,ts2)) = 
        let
            (* given two lists of types, combines into a conjunction list of simple equalities *)
            fun c_list [] [] = TRIVIAL
              | c_list (x::xs) (y::ys) = (x ~ y /\ (c_list xs ys))
              | c_list _  _  = raise BugInTypeInference "invalid use of conjunction list"
        in solve (n1 ~ n2 /\ c_list ts1 ts2)
        end
  | solve (t1 ~ t2)  = unsatisfiableEquality(t1, t2) (* not valid simple equaility - no solution *)
...
(* function [[literal]], to infer the type of a literal constant ((prototype)) 421c *)
      fun literal (NIL)    = (listtype alpha, TRIVIAL)
        | literal (BOOL b) = (booltype, TRIVIAL)
        | literal (NUM  n) = (inttype, TRIVIAL)
        | literal (SYM  s) = (symtype, TRIVIAL)
        | literal (PAIR (v, vs)) = 
            let
                (* parse v and check if vs is NIL or actual list - if list, parse vs *)
                val (t1, c1) = literal v
                val (t2, c2) = case vs of NIL => (listtype t1, TRIVIAL) (* simple tuple to pass eqType*)
                                        | _   => literal vs
            in if eqType(listtype t1, t2)
                then (t2, c1 /\ c2)
                else unsatisfiableEquality(t1, t2)
            end
        | literal _  = raise BugInTypeInference "invalid literal case"
      ...
      fun ty (LITERAL n) = literal n
        ...
        | ty (BEGIN es) =
            let
                ...
                val tau = foldl (fn (t,n) => t) unittype ts (* get final exp's type *)
            in (tau, cs)
            end
        | ty (LAMBDA (formals, body)) = 
            let
                (* extend gamma with each x_i bound to freshtyvar, get type of body using extended gamma *)
                val gamma_ext = foldl (fn (n, env) => bindtyscheme (n, FORALL([n],freshtyvar()), env)) Gamma formals
                val (tau, c)  = typeof(body, gamma_ext)

            (* return function type with freshtyvars as formals and the type and constraint of the body *) 
            in (funtype(map (fn (n) => freshInstance(findtyscheme (n, gamma_ext))) formals, tau), c)
            end

        (* LET & LETREC follow the inference rules from Ramsey p.438 *)
        | ty (LETX (LET, bs, body)) =
            let
                val (ns, es) = ListPair.unzip bs 
                val (ts, c)  = typesof(es, Gamma)
                val theta    = solve(c)
                val env      = inter(dom theta, freetyvarsGamma Gamma)
                val c'       = foldl (fn (alpha, c) => c /\ (TYCON alpha) ~ tysubst theta (TYCON alpha)) TRIVIAL env
                val sigmas   = map (fn (tau) => generalize(tysubst theta tau, union(freetyvarsGamma Gamma, freetyvarsConstraint c'))) ts
                val gamma_ext  = foldl (fn ((n,s),env) => bindtyscheme (n, s, env)) Gamma (ListPair.zip (ns, sigmas))
                val (tau, c_b) = typeof(body, gamma_ext)
            in (tau, c' /\ c_b)
            end
        | ty (LETX (LETREC, bs, body)) =
            let
                val (ns, es)  = ListPair.unzip bs
                val gamma'    = foldl (fn (n, env) => bindtyscheme (n, FORALL([n],freshtyvar()), env)) Gamma ns
                val (ts, c_r) = typesof(es, gamma')
                val alphas    = map (fn (x) => (freshInstance(findtyscheme (x, gamma')))) ns
