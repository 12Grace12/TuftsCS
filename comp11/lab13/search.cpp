...
int search_parallel_arrays(int nums[], string *words[])
{
	...
	for (int i=0;i<ARRAY_SIZE;i++){
	    if (words[i] != NULL && *words[i] == SECRET){
		sum += nums[i];
	    }
	}
...
int search_array_of_structs(Item list[])
{
	int sum = 0;
	for (int i=0;i<ARRAY_SIZE;i++){
	    if (list[i].wordptr != NULL && *(list[i].wordptr) == SECRET){
		sum += list[i].num;
	    }
	}
...
int search_array_of_ptrs_to_structs(Item *list[])
{
	...
	for (int i=0;i<ARRAY_SIZE;i++){
	    if (list[i] != NULL){
		Item elem = *list[i];
		if (elem.wordptr != NULL){
		    if (*elem.wordptr == SECRET){
			sum += elem.num;
		    }
		}
	    }
...
int search_array_of_ptrs_to_arrays(Item *list[])
{
	...
	for (int i=0;i<ARRAY_SIZE;i++){
	    if (list[i] != NULL){
		int n = 0;
		while (((list[i])[n]).wordptr != NULL){
		    Item elem_array = (list[i])[n];
		    if (elem_array.wordptr != NULL && *(elem_array.wordptr) == SECRET)
			sum += elem_array.num;
		    n++;
		}
	    }
...
int search_linkedlist(Link *head)
{
	...
	Link *temp = head;
	while (temp != NULL){
	    if ((temp->data).wordptr != NULL && *((temp->data).wordptr) == SECRET){
		sum += (temp->data).num;
	    }
	    temp = (temp->next);
	}
